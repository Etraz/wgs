...
Game BridgeFactory::make(std::unique_ptr<AbstractSendRec> &&sendRec,
    ...
    auto currentPlayerStartsNewTrickAction = std::make_shared<CurrentPlayerStartsNewTrickAction>(CurrentPlayerStartsNewTrickAction{});
    ...
    edges->at(0).push_back(Edge("", bridgeDealAction, alwaysTrueCondition, 1));
    edges->at(1).push_back(Edge("", startTrickAction, handIsNotEmptyCondition, 2));
    edges->at(1).push_back(Edge("", restartAction, handIsEmptyCondition, 0));
    edges->at(2).push_back(Edge("", playNextCardAction, currentTrickIsNotFullCondition, 2));
    edges->at(2).push_back(Edge("", endTrickAction, currentTrickIsFullCondition, 1));
